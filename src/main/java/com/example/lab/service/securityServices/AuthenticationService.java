...
public class AuthenticationService {
	...
	public void register(UserRegisterRequest request, String uri) throws Throwable {
		...
		try {// save user
			...
			if (request.getRole().equals(Roles.BUDDY.getVal())) {
				emailServiceImpl.sendRegisterEmail(userRepository.getAdminEmail(Roles.ADMIN.getVal()),
						request.getUsername(), jwtToken, uri, request.getEmail(), Roles.BUDDY.getVal());
			} else {
				emailServiceImpl.sendRegisterEmail(request.getEmail(), request.getUsername(), jwtToken, uri,
						request.getEmail(), Roles.USER.getVal());
			}
		} catch (Exception e) {
...
//		    }
		...
		Token token = Token.builder().userId(user.getId()).token(jwtToken).expired(false).revoked(false).build();
		tokenRepository.save(token);
		UserDto userDto = UserDto.builder().username(user.getUsername()).password(null).role(user.getRole()).build();
		return AuthenticationResponse.builder().userDto(userDto).token(jwtToken).timeAlive(JwtService.TIME_ALIVE)
				.build();
	}
	...
	public boolean verifyConfirm(String token) throws Exception {
		...
		if (!user.isEmpty()) {
			...
			activeUser.setUpdateAt(LocalDateTime.now());
			userRepository.save(activeUser);
			emailServiceImpl.sendActiveAccountEmail(activeUser.getEmail(),
